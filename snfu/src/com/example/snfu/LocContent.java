...
public class LocContent extends AsyncTask<Void, Void, Void> {
	...
	public String nameencode(String search_st, int data_num) {
		try {
			// 위치 변환할 주소 인코딩
			search = URLEncoder.encode(search_st, "UTF-8");
			// 연결할 사이트 주소 선택
			uri = "http://openAPI.seoul.go.kr:8088/" + api_key
					+ "/xml/SearchCulturalFacilitiesAddressService/1/"
					+ data_num + "/" + search + "/";
			...
			while (eventType != XmlPullParser.END_DOCUMENT) {
				if (eventType == XmlPullParser.START_TAG) {
					...
					if (tag_name.equals("FAC_NAME") && isInRowTag) {
						...
						codename += xpp.getText(); // name에 해당하는 모든 텍스트를
													// 읽어드림(+=)
